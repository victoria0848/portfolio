    ...
    <>
      <header className="bg-white dark:bg-gray-950 shadow-sm sticky top-0 z-50">
        <nav className="max-w-6xl mx-auto px-6 py-5 flex justify-between items-center">
          ...
          <div className="flex gap-8 text-sm font-medium text-gray-700 dark:text-gray-300">
            <a href="#about" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Om mig</a>
            <a href="#skills" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Skills</a>
            <a href="#projects" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Projekter</a>
            <a href="#contact" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Kontakt</a>
          </div>
          <button
            onClick={() => setDarkMode(!darkMode)}
            className="px-4 py-2 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-full text-sm font-medium hover:scale-110 transition-all duration-200"
          >
            {darkMode ? '☀️ Lys mode' : '🌙 Mørk mode'}
          </button>
        </nav>
      </header>
      ...
      <main className="min-h-screen font-sans antialiased">
        {/* HERO */}
        <section className="bg-gradient-to-br from-blue-600 to-indigo-700 dark:from-gray-950 dark:to-black text-white py-24">
          <div className="max-w-4xl mx-auto text-center px-6">
            <h2 className="text-5xl md:text-6xl font-bold tracking-tight">Hej, jeg er {name}</h2>
            <p className="mt-4 text-xl md:text-2xl opacity-90">Webudvikler • Lærer Tailwind denne uge</p>
            <button
              onClick={scrollToProjects}
              className="mt-8 bg-white text-blue-600 dark:bg-gray-800 dark:text-gray-200 px-8 py-4 rounded-2xl font-semibold hover:bg-yellow-300 dark:hover:bg-yellow-400 hover:scale-105 transition-all duration-300"
            >
              Se mine projekter ↓
            </button>
          </div>
        ...
        {/* CONTACT */}
        <section id="contact" className="bg-gray-900 dark:bg-black text-white py-20">
          <div className="max-w-2xl mx-auto px-6 text-center">
            <h2 className="text-4xl font-bold mb-8 text-white">Kontakt mig</h2>
            <form className="space-y-6">
              ...
              <textarea
               placeholder="Besked..."
        rows={5}                    // fast antal rækker (højde)
        className="w-full p-4 rounded-2xl bg-gray-800 border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"  
              />
              <button className="w-full bg-blue-600 hover:bg-blue-500 dark:bg-blue-700 dark:hover:bg-blue-600 py-4 rounded-2xl font-semibold transition duration-300 text-white">
                Send besked
              </button>
            </form>
          </div>
      ...
      <footer className="text-center py-8 text-sm text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-950">
        Lavet med ❤️ og Tailwind CSS – 2026
      </footer>
    </>
  );
}
