  ...
  return (
    <>
      ...
      <nav className="max-w-6xl mx-auto px-6 py-5 flex justify-between items-center">
        ...
        aria-label="Tilbage til toppen">Portfolio</a>

        {/* Nav */}
        <ul className="flex gap-8 text-sm font-medium text-purple-700 dark:text-gray-300">
          <li><a href="#about" className="hover:text-purple-900 dark:hover:text-purple-400 transition">Om mig</a></li>
          <li><a href="#projects" className="hover:text-purple-900 dark:hover:text-purple-400 transition">Projekter</a></li>
          <li><a href="#contact" className="hover:text-purple-900 dark:hover:text-purple-400 transition">Kontakt</a></li>
        </ul>

        {/* Dark mode */}
        <button
          onClick={() => setDarkMode(!darkMode)}
          className="px-4 py-2 bg-purple-200 dark:bg-purple-800 text-purple-800 dark:text-gray-200 rounded-full text-sm font-medium hover:scale-110 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-purple-500"
          aria-label={darkMode ? 'Skift til lys mode' : 'Skift til mørk mode'}>
      ...
      <main className="min-h-screen font-sans antialiased">
        ...
        <figure className="grid md:grid-cols-2 gap-12 items-center">
          <img
            src="https://picsum.photos/600/400"
            className="rounded-2xl shadow-xl w-full object-cover"
            alt="Profilbillede af mig selv ved computeren"
          />

          <figcaption className="space-y-6 text-lg text-gray-900 dark:text-gray-200">
        ...
        <ul className="grid md:grid-cols-3 gap-8">
          {[1, 2, 3].map((i) => (
            <li key={i}>
              <article className="group bg-white dark:bg-gray-900 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-200 dark:border-gray-800 h-full flex flex-col">
                <figure className="overflow-hidden">
                  <img
                    src={`https://picsum.photos/id/${i + 10}/600/400`}
                    className="w-full h-56 object-cover group-hover:scale-110 transition-transform duration-500"
                    alt={`Skærmbillede fra projekt ${i}`}
                  />
                </figure>
